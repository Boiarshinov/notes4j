...
Драйверый для других языков - просто обертки над джавовым.
...
Стратегии компакции:
- `Size-Tiered Compation Strategy` (STCS) - Используется по умолчанию
- `Leveled Compation Strategy` (LCS) - данные сортируются между sstable. Файлы организуются в уровни. Но на каждом уровне первичный ключ может встретиться только один раз. Такой подход похож на B-дерево.
