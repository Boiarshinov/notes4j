...
Каждая партиция (а вернее ее последний сегмент) - это файл, поэтому для каждой партиции на брокере ему приходится держать открытым файловый дескриптор.
...
Каждая партиция представляет собой набор файлов, называемых __сегментами__.
Когда текущий сегмент заполняется данными, он закрывается, а Кафка начинает писать партицию в новый сегмент.
Размер сегмента определяется настройками и по умолчанию равен 1ГБ.
...
Можно задать только один из них, либо оба одновременно, либо хранить данные в Kafka вечно.

Kafka раз в некий конфигурируемый промежуток времени проверяет не надо ли ей почистить лог.
По умолчанию она это делает раз в 5 минут.
Удаление записей из партиции происходит посегментно.
Если удаление происходит по времени, то Kafka смотрит на timestamp последней записи в сегментах и исходя из этого определяет какие сегменты подлежат удалению.

### Compaction
Помимо удаления старых записей Kafka позволяет уменьшать размер партиций за счет процесса __compaction__.
При включенном compaction Kafka периодически удаляет из партиции дубликаты одного ключа, оставляя только последнее записанное значение.
Офсеты записей при этом не меняются, просто в массиве записей появляются дыры.

Считается, что compaction - это довольно трудоемкий процесс, т.к. ему приходится изменять старые сегменты партиций, которые обычно являются немутируемыми.
Поэтому данный механизм используется довольно редко и как правило для случаев, когда необходимо хранить снепшоты данных.
Примером использования compaction является [Schema Registry](./kafka_schema_registry.md)
...
__Продюсеры__ пишут в Kafka, __консьюмеры__ - читают из нее.
Продюсеры и консьюмеры представлены в виде клиентских библиотек для различных языков программирования, но при этом основным является клиент для Java.

Общение между клиентами и Kafka ведется по собственному протоколу, который построен поверх [TCP](../network/tcp.md).


### Продюсеры
Продюсеры имеют локальный буфер, в котором они накапливают сообщения, прежде чем отправить их в Kafka.
Отправка ведется отдельным (и всего одним) потоком.
Частота отправки сообщений регулируется с помощью параметра `linger.ms`.
При этом если буфер заполнится до истечения времени, то весь батч будет отправлен немедленно.

После отправки сообщения в Kafka продюсер ожидает он нее подтверждения.
Это поведение можно конфигурировать с помощью параметра `acks`.
- `0` - продюсер вообще не ждет ответа от Kafka. Если Kafka не получила сообщение, то оно потерялось
- `1` - продюсер считает сообщение отправленным, если он получил ответ от ведущей реплики
- `all` - продюсер считает сообщение отправленным, если он получил ответ от всех In-Sync реплик.

По умолчанию продюсеры могут иногда дублировать записи, если при первой отправке они не получили ack от Kafka.
Чтобы избежать этого, можно включить __идемпотентность на продюсере__.
В этом случае продюсер вместе с каждой записью отправляет в Kafka свой уникальный `producerId` (PID) и монотонно возрастающий `sequenceNumber` (SEQ). 
Если Kafka получит два сообщения с одинаковым PID и SEQ, то она сообщит продюсеру, что тот прислал дубликат.
Этот же механизм позволяет избежать реординга сообщений.
Идемпотентность включается с помощью настройки `enable.idempotence=true` на продюсере, но для этого должен быть выставлен параметр `acks=all`.
...
Офсеты консьюмер групп для каждой партиции хранятся в самой Kafka в специальном топике `__consumer_offsets`.
Когда консьюмер хочет сдвинуть офсет, он сообщает об этом Кафке, этот процесс называется __коммитом офсета__.
