...
В distributed режиме развертывания Kafka Connect хранит системные данные в трех топиках Kafka, задаваемых настройками:
- `config.storage.topic` - хранение конфигураций коннекторов и тасок. Также сюда регулярно прописывается ключ сессии авторизации
- `offset.storage.topic` - оффсеты source-коннекторов. Хранят значения не оффсета Kafka, а смещения в источнике данных. Например, если source подключен к реляционной БД и вычитка идет по времени создания записи, то в топике будут храниться значения timestamp за который происходила выборка. Оффсеты sink-коннекторов хранятся в Kafka в системном топике `__consumer_offsets`
- `status.storage.topic` - события изменения статуса тасок и коннекторов

По умолчанию все данные хранятся в топиках в формате JSON.

Топики будут созданы автоматически при запуске Kafka Connect, но лучше сконфигурировать их предварительно самостоятельно.
- `config.storage.topic` - Должен иметь одну партицию и высокий фактор репликации. Должна быть включена компакция. Ограничение в одну партицию связано с тем, что конфигурация жестко завязана на последовательность сообщений в топике
- `offset.storage.topic` - Должен иметь множество партиций. Должен иметь реплики. Должна быть включена компакция.
- `status.storage.topic` - Должен иметь множество партиций. Должен иметь реплики. Должна быть включена компакция.

Множество партиций нужно для параллелизации работы тасок.
Похоже, тасок не может быть больше, чем партиций в `offset.storage.topic`.

Коннектор читает из Kafka под собственной consumer-группой.
...
- [X] [Официальная документация](https://kafka.apache.org/documentation/#connect)
...
- [X] [Частые ошибки при настройке Kafka Connect и рекомендации по их устранению](https://www.confluent.io/blog/kafka-connect-deep-dive-converters-serialization-explained/) - 2018 г.
